...
export function LoginForm({ admin = false }: { admin?: boolean }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setError('')
    ...
    setLoading(true); try { const user = await login({ email, password }); if (admin && user.role !== 'ADMIN') { logout(); setError('This account does not have admin access.'); return } const from = (location.state as { from?: { pathname?: string } } | null)?.from?.pathname; navigate(admin ? ROUTES.adminDashboard : from ?? (user.role === 'ADMIN' ? ROUTES.adminDashboard : ROUTES.userDashboard), { replace: true }) } catch { setError('Invalid email or password.') } finally { setLoading(false) }
  }
